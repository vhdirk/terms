...
mod imp {
	use std::{
		cell::{Cell, RefCell},
		path::Path, collections::HashMap, ops::Deref, borrow::{Cow, Borrow}, env,
	};

	use gio::Cancellable;
	use glib::{
		clone, subclass::Signal, JoinHandle, ObjectExt, Pid, Properties,
		SignalHandlerId, SpawnFlags, StaticType, Type, Value,
	};
	use gtk::{GestureClick, ffi::gtk_application_get_active_window};
    use once_cell::sync::Lazy;
	use shell_quote::Sh;
    use vte::{
		CancellableExt, PtyFlags, TerminalExt, TerminalExtManual, WidgetExt, PopoverExt,
	};
	...
	impl Terminal {
		...
		fn setup_drag_drop(&self) {
			...
			let drop_handler_id = drop_target.connect_drop(
				clone!(@weak self as this => @default-return true, move |_, value, _, _| {
                    this.on_drop(value)
				}),
			);
		...
		fn connect_signals(&self) {
			let handler = self.term.connect_child_exited(
				clone!(@weak self as this => move |_, status| {
            ...
            click.connect_pressed(
                clone!(@weak self as this => move |_: &gtk::GestureClick, _: i32, x: f64, y: f64| {
                    this.show_menu(x, y);
				}),
            );
...
impl Terminal {
	...
	pub fn connect_exit<F: Fn(&Self, i32) + 'static>(
		&self,
		f: F,
