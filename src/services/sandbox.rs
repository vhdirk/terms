use async_std::{fs::File, path::PathBuf, prelude::*};
...
pub async fn is_sandboxed() -> bool {
    ...
    let new_value = is_flatpak().await || is_snap().await || std::env::var("GTK_USE_PORTAL").map(|v| v == "1").unwrap_or(false);
...
mod tests {
    ...
    fn test_cgroup_v2_is_snap() {
        let data = "0::/user.slice/user-1000.slice/user@1000.service/apps.slice/snap.something.scope\n";
